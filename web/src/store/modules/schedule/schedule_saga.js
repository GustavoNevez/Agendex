...
import {
  updateAgendamento,
  updateServicos,
  updateDiasDisponiveis,
  updateClientes,
  updateProfissionais,
  deleteAgendamentoSuccess,
  finalizeAgendamentoSuccess,
  updateAgendamentosSemana,
} from "./schedule_action";
...
export function* fetchProximosSeteDias({ estabelecimentoId }) {
  try {
    ...
    const { data: response } = yield call(
      api.get,
      `/agendamento/proximos-sete-dias/${estabelecimentoId}`
    );
...
export function* filterAgendamento({ start, end }) {
  try {
    ...
    const agendamentosAtivos = response.agendamentos.filter(
...
export function* fetchServicos() {
  try {
    ...
    const servicosAtivos = response.servicos.filter(
      (servico) => servico.status === "A"
    );
...
export function* fetchDiasDisponiveisProfissional({
  estabelecimentoId,
  data,
  servicoId,
  profissionalId,
...
export function* fetchProfissionais() {
  try {
    ...
    const profissionaisAtivos = response.profissionais.filter(
      (profissional) => profissional.status === "A"
    );
...
export default all([
  ...
  takeLatest(
    types.FETCH_DIAS_DISPONIVEIS_PROFISSIONAL,
