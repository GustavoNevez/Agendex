...
export default function CustomDatePicker({ onChange, disabledDate }) {
        ...
        <div className="grid grid-cols-7">
          {weekDays.map((day, index) => (
            <button
              ...
              className={`
                py-6 relative
                ${day.isDisabled ? 'cursor-not-allowed bg-gray-100' : 'cursor-pointer'}
                ${day.isSelected ? 'bg-violet-500' : day.isDisabled ? 'bg-gray-100' : 'bg-white'}
                ${!day.isSelected && !day.isDisabled ? 'hover:bg-violet-50' : ''}
                transition-colors duration-200 focus:outline-none
              `}
            >
              <div className="flex flex-col items-center">
                <span className={`text-xl font-semibold 
                  ${day.isSelected ? 'text-white' : day.isDisabled ? 'text-gray-400' : 'text-gray-700'}`}>
                  {day.dayNumber}
                </span>
                <span className={`text-xs mt-1 
                  ${day.isSelected ? 'text-white' : day.isDisabled ? 'text-gray-400' : 'text-violet-500'}`}>
                  {day.month}
                </span>
