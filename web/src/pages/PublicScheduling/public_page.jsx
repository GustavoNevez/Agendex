...
const PublicScheduling = () => {
    ...
    const { publicData, availability, clientRegistration, lastAppointmentCreated,
        appointmentCreationSuccess, } = useSelector((state) => state.public);
    ...
    useEffect(() => {
        ...
        const selectedService = publicData?.servicos?.find(
            s => s.id === lastAppointmentCreated.servicoId || s._id === lastAppointmentCreated.servicoId
        ) || (formState?.servicoId && publicData?.servicos?.find(
            s => s.id === formState.servicoId || s._id === formState.servicoId
        ));

        const selectedProfissional = type === 'p'
            ? publicData?.profissional
            : publicData?.profissionais?.find(
                p => p.id === lastAppointmentCreated.profissionalId || p._id === lastAppointmentCreated.profissionalId
            ) || (formState?.profissionalId && publicData?.profissionais?.find(
                p => p.id === formState.profissionalId || p._id === formState.profissionalId
            ));
