...
// Função para calcular a duração do serviço a partir de um horário UTC
function getServiceDurationFromUtc(utcDateString) {
    if (!utcDateString) return '';
    // O horário UTC representa a duração do serviço a partir da meia-noite UTC-3
...
// Ajusta o horário UTC para o horário local (diminuindo 3 horas)
...
const PublicScheduling = () => {
    ...
    const [reservasAuthStep, setReservasAuthStep] = useState(0); // 0: escolha, 1: login, 2: registro
    ...
    useEffect(() => {
        if (appointments && appointments.length > 0) {
            ...
            // Busca o serviço e profissional pelo próprio agendamento, não pelo formState
            let selectedService =
                publicData.servicos.find(
                    s =>
                        s.id === (lastAppointment.servicoId?._id || lastAppointment.servicoId) ||
                        s._id === (lastAppointment.servicoId?._id || lastAppointment.servicoId)
                );
            let selectedProfissional =
                type === 'p'
                    ...
                    : publicData.profissionais.find(
                        p =>
                            p.id === (lastAppointment.profissionalId?._id || lastAppointment.profissionalId) ||
                            p._id === (lastAppointment.profissionalId?._id || lastAppointment.profissionalId)
                    );

            // Fallback: se não encontrar, tenta pelo formState
            if (!selectedService && formState.servicoId) {
                selectedService = publicData.servicos.find(s => s.id === formState.servicoId || s._id === formState.servicoId);
            }
            if (!selectedProfissional && formState.profissionalId) {
                selectedProfissional =
                    type === 'p'
                        ? publicData.profissional
                        : publicData.profissionais.find(p => p.id === formState.profissionalId || p._id === formState.profissionalId);
            }

            // Debug
            // console.log('lastAppointment', lastAppointment, selectedService, selectedProfissional);
    ...
    // Função para resetar o formulário ao trocar entre registrar/login nas reservas
    const resetReservasForm = (registrar) => {
        setFormState({
            nome: '',
            email: '',
            telefone: '',
                ...
                <div className="flex items-center justify-between mb-2">
                    <div >
                        ...
                        <p className="text-sm mt-1">
                            com <span className="font-semibold text-gray-800">{selectedProfissional}</span>
                        </p>
                    </div>
    ...
    const renderSuccessStep = () => {
        ...
        // Use fallback para garantir que sempre mostra algo
    ...
    const renderTimeBlocks = () => {
        ...
        if (!loading && hasFetchedAvailability === true && availableHours.length === 0) {
            return (
